...
# ✅ Correct Flask app path with templates directory
...
def fetch_nearby_medical_stores(lat, lon):
    query = f"""[out:json]; node["amenity"="pharmacy"](around:5000,{lat},{lon}); out;"""
...
key_structures = {
    0: "Thalami", 1: "Midbrain", 2: "Palate", 3: "4th Ventricle",
    4: "Cisterna Magna", 5: "NT", 6: "Nasal Tip", 7: "Nasal Skin", 8: "Nasal Bone"
}
...
def analyze():
    try:
        ...
        image_height, _ = image_rgb.shape[:2]
        ...
        structure_status = {name: ("Detected" if name in detected_structures else "Not Detected") for name in key_structure_names}
        ...
        result_data = {
            ...
            "summary": f"Nuchal Translucency: {nt_measurement_mm or 'Not Detected'} mm",
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
